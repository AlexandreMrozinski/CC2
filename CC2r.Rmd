...
samdf <- data.frame(Profondeur=profondeur, Mois=day)
...
```{r}
plot_richness(ps, x="Mois", measures=c("Shannon", "Simpson"), color="Saison")
```
...
```
Question: 
Comment les communautées sont impactées par la profondeur et la période d'échantillobnage ? 